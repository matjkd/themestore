<?php
	Class Customer_model extends CI_Model
	{
		...
		function login($email, $password, $remember = false)
		{
			...
			if ($customer)
			{
				...
				$this -> db -> where(array(
					...
					'id' => $customer['default_billing_address']
				));
				...
				if ($address)
				{
					...
					$customer['bill_address']['id'] = $address['id'];
					// save the addres id for future reference
				}

				$this -> db -> where(array(
					'customer_id' => $customer['id'],
					'id' => $customer['default_shipping_address']
				));
				$address = $this -> db -> get('customers_address_bank') -> row_array();
				if ($address)
				{
		...
		function is_logged_in($redirect = false, $default_redirect = 'secure/login/')
		{
			...
			//$redirect allows us to choose where a customer will get redirected to after
			// they login
			//$default_redirect points is to the login page, if you do not want this, you can
			// set it to false and then redirect wherever you wish.
		...
		function set_referrer($email, $referrer_id)
		{
